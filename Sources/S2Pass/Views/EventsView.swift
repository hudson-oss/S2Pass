...
struct EventsListView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                Text("SELECT AN EVENT TO PURCHASE TICKETS").font(.title3).fontWeight(.bold)
                Spacer()
                Button("FILTER") {}
                    .fontWeight(.bold)
...
struct EventRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(event.sportName).font(.headline)
                Spacer()
                Text(event.ticketsOnSale ? "AVAILABLE" : "NOT AVAILABLE")
                    .font(.subheadline)
                    .fontWeight(.bold)
            ...
            if !event.opponent.isEmpty {
                HStack(spacing: 6) {
                    Text(event.isHomeGame ? "HOME" : "AWAY")
                        .font(.caption)
                        .fontWeight(.bold)
                    ...
                    Text("VS")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text(event.isHomeGame ? event.opponent : event.location)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text("|")
                    Text(event.reservedSeatingAvailable ? "Reserved Seating" : "No Reserved Seating")
                        .font(.caption)
                        .foregroundStyle(event.reservedSeatingAvailable ? Theme.schoolColor : .secondary)
                }
            }
